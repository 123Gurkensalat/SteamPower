...
internal class GuiDialogSteampot : GuiDialogBlockEntity
{
    ...
    public GuiDialogSteampot(string dialogTitle, InventoryBase inventory, BlockPos blockEntityPos, ICoreClientAPI capi)
    ...
    private string getContentsText()
    {
        ...
        else
        {
            ...
            BlockEntitySteampot blockEntityBarrel = capi.World.BlockAccessor.GetBlockEntity(base.BlockEntityPosition) as BlockEntitySteampot;
    ...
    private void fullnessMeterDraw(Context ctx, ImageSurface surface, ElementBounds currentBounds)
    {
        ...
        if (!itemSlot.Empty)
        {
            BlockEntitySteampot obj = capi.World.BlockAccessor.GetBlockEntity(base.BlockEntityPosition) as BlockEntitySteampot;
            float num = 1f;
    ...
    {
        if (!(capi.World.BlockAccessor.GetBlockEntity(base.BlockEntityPosition) is BlockEntitySteampot { Sealed: false } blockEntityBarrel))
        {
            return true;
        }
